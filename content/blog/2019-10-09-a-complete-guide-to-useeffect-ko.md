...
- [함수도 데이터 흐름의 일부인가?](#%ed%95%a8%ec%88%98%eb%8f%84-%eb%8d%b0%ec%9d%b4%ed%84%b0-%ed%9d%90%eb%a6%84%ec%9d%98-%ec%9d%bc%eb%b6%80%ec%9d%b8%ea%b0%80)
...
![요다가 공기를 들이마시고 있다. "베이컨 냄새가 나는군."](./images/a-complete-guide-to-useeffect/yoda.jpg)
...
![카운터 데모](./images/a-complete-guide-to-useeffect/counter.gif)
...
![1, 2, 3, 4, 5가 순서대로 로딩되는 화면 녹화](./images/a-complete-guide-to-useeffect/timeout_counter.gif)
...
![5, 5, 5, 5, 5가 순서대로 로깅되는 화면 녹화](./images/a-complete-guide-to-useeffect/timeout_counter_class.gif)
...
![5, 5, 5, 5, 5가 순서대로 로깅되는 화면 녹화](./images/a-complete-guide-to-useeffect/timeout_counter_refs.gif)
...
![데자뷰 (영화 매트릭스의 고양이 등장 씬)](./images/a-complete-guide-to-useeffect/deja_vu.gif)
...
![이펙트가 하나를 다른 것으로 바꾸는 다이어그램](./images/a-complete-guide-to-useeffect/deps-compare-correct.gif)
...
![이펙트가 하나를 다른 것으로 바꾸는 다이어그램](./images/a-complete-guide-to-useeffect/deps-compare-wrong.gif)
*(의존성이 같으므로 이펙트는 스킵한다.)*
...
![잘못된 인터벌 클로저의 다이어그램](./images/a-complete-guide-to-useeffect/interval-wrong.gif)
*(의존성이 같으므로 이펙트는 스킵한다.)*
...
![재구독하는 인터벌의 다이어그램](./images/a-complete-guide-to-useeffect/interval-rightish.gif)
*(의존성이 다르기 때문에 이펙트를 다시 실행한다)*
...
![제대로 작동하는 인터벌의 다이어그램](./images/a-complete-guide-to-useeffect/interval-right.gif)
*(의존성은 같으므로 이펙트는 스킵한다.)*
...
![린트 규칙 gif](./images/a-complete-guide-to-useeffect/exhaustive-deps.gif)
...
이제 여러분은 제가 알고있는 이펙트에 대한 정보 거의 전부를 알게 되었습니다, 다시 시작 부분으로 돌아가 [요약본](#tldr-too-long-didnt-read---%ec%9a%94%ec%95%bd)을 읽어 보세요. 이해가 되시나요? 아니면 제가 뭘 빠뜨렸을까요? (아직 손 끝의 잉크가 마르지 않았어요!)

[트위터](https://mobile.twitter.com/dan_abramov)에서 의견을 주시면 좋겠습니다! 읽어주셔서 고맙습니다.
