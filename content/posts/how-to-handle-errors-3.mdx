      ...
      <main className="main">
        ...
        <ul>
          {store.todoItems.map((todo) => (
            <li className={todo.isDone ? 'done' : ''} key={todo.id} onClick={todo.toggleDone}>
              {todo.title}
            </li>
          ))}
        </ul>
...
export const FetchingStore = types
  ...
  .views((self) => {
    function getActionStatus(actionName: string): IFetchingModel {
      ...
      if (!actionStatus) {
        throw new Error(`Could not find action status from actionName: ${actionName}`)
      }
      return actionStatus
    }
  ...
  .actions((self) => ({
    ...
    setStatus(actionName: string, status: FetchingStatus, error?: { message: string }) {
...
type FlowFunction<R, Args extends any[]> = (...args: Args) => Generator<any, R | undefined, any>
